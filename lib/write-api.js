...
function writeApiGen(Network, network, structs, config, schemaDef) {
  ...
  var reserveFunctions = new _set2.default(['transaction', 'contract']);
  var merge = {};
  ...
  var genTransaction = function genTransaction(structs, merge) {
    return async function () {
      ...
      if (isContractArray) {
        ...
        contracts = [args[0]];
        args = args.slice(1);
      } else if ((0, _typeof3.default)(args[0]) === 'object' && (0, _typeof3.default)(Array.isArray(args[0].actions))) {
        // full transaction, lookup ABIs used by each action
        var accounts = new _set2.default(); // make a unique list
        var _iteratorNormalCompletion = true;
        var _didIteratorError = false;
        var _iteratorError = undefined;

        try {
          for (var _iterator = (0, _getIterator3.default)(args[0].actions), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
        ...
        await _promise2.default.all(abiPromises);
      }
      ...
      if (contracts) {
        ...
        try {
          for (var _iterator2 = (0, _getIterator3.default)(contracts), _step2; !(_iteratorNormalCompletion2 = (_step2 = _iterator2.next()).done); _iteratorNormalCompletion2 = true) {
        ...
        return _promise2.default.all(contractPromises).then(function (actions) {
          var merges = {};
  ...
  function genMethod(type, definition, transactionArg) {
    ...
    return function () {
      ...
      if ((typeof lastArg === 'undefined' ? 'undefined' : (0, _typeof3.default)(lastArg)) === 'object' && (0, _typeof3.default)(lastArg.__optionOverrides) === 'object') {
        ...
        (0, _assign2.default)(optionOverrides, args.pop().__optionOverrides);
      }

      var processedArgs = processArgs(args, (0, _keys2.default)(definition), type, optionsFormatter);
      ...
      };options = (0, _assign2.default)({}, optionDefaults, options, optionOverrides);
      ...
      if (options.authorization) {
        ...
        assert.equal(authorization.length, options.authorization.length, 'invalid authorization in: ' + (0, _stringify2.default)(options.authorization));
      }
      ...
      if (addDefaultAuths) {
        var fieldKeys = (0, _keys2.default)(definition);
        var f1 = fieldKeys[0];
  ...
  */
  function trMessageCollector(trCallback) {
    ...
    try {
      ...
      promiseCollector = _promise2.default.reject(error);
    }

    return _promise2.default.resolve(promiseCollector).then(function () {
      return _promise2.default.all(messageList).then(function (resolvedMessageList) {
        var actions = [];
        ...
        try {
          for (var _iterator3 = (0, _getIterator3.default)(resolvedMessageList), _step3; !(_iteratorNormalCompletion3 = (_step3 = _iterator3.next()).done); _iteratorNormalCompletion3 = true) {
            var m = _step3.value;
  ...
  function transaction(arg, options, callback) {
    ...
    options = (0, _assign2.default)({} /*clone*/, optionDefault, options);

    var returnPromise = void 0;
    if (typeof callback !== 'function') {
      returnPromise = new _promise2.default(function (resolve, reject) {
    ...
    headers(options.expireInSeconds, checkError(callback, async function (rawTx) {
      ...
      rawTx = (0, _assign2.default)({}, rawTx);
      ...
      var tr = Transaction.toObject(txObject);
      ...
      _promise2.default.all(sigs).then(function (sigs) {
...
function usage(type, definition, Network, account, config) {
  ...
  if (account === 'eosio' || account === 'eosio.token') {
    var _Structs = Structs((0, _assign2.default)({ defaults: true, network: Network }, config)),
        structs = _Structs.structs;

    struct = structs[type];

    out('PARAMETERS');
    out((0, _stringify2.default)(definition, null, 4));
    out();

    out('EXAMPLE');
    out((0, _stringify2.default)(struct.toObject(), null, 4));
  } else {
    var cache = config.abiCache.abi(account);
    out('PARAMETERS');
    out((0, _stringify2.default)(schemaFields(cache.schema, type), null, 4));
    out();
...
var checkError = function checkError(parentErr, parrentRes) {
  return function (error, result) {
    if (error) {
      ...
      _promise2.default.resolve(parrentRes(result)).catch(function (error) {
        parentErr(error);
      });
    }
  };
...
function schemaFields(schema, type) {
  ...
  var def = {};
  if (base && base !== '') {
    (0, _assign2.default)(def, schemaFields(schema, base));
  }
  (0, _assign2.default)(def, fields);
  return def;
}