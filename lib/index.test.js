...
describe('offline', function () {
  ...
  it('transaction', async function () {
    ...
    var memo = '';
    var trx = await eos.transfer('bankers', 'people', '1000000 SYS', memo);
...
if (process.env['NODE_ENV'] === 'development') {
  ...
  describe('Contracts', function () {
    it('Messages do not sort', async function () {
      ...
      var tx = await local.transaction(['currency', 'eosio.token'], function (_ref) {
  ...
  describe('Contract', function () {
    function deploy(contract) {
      ...
      it('deploy ' + contract + '@' + account, async function () {
        ...
        var config = { binaryen: require("binaryen"), keyProvider: wif };
        var eos = Eos(config);

        var wasm = fs.readFileSync('docker/contracts/' + contract + '/' + contract + '.wasm');
        var abi = fs.readFileSync('docker/contracts/' + contract + '/' + contract + '.abi');

        await eos.setcode(account, 0, 0, wasm);
        await eos.setabi(account, JSON.parse(abi));

        var code = await eos.getCode(account);

        var diskAbi = JSON.parse(abi);
  ...
  describe('Contracts Load', function () {
    function load(name) {
      it(name, async function () {
        var eos = Eos();
        var contract = await eos.contract(name);
  ...
  describe('transactions', function () {
    ...
    var promiseSigner = function promiseSigner(args) {
      return _promise2.default.resolve(signProvider(args));
    };
    ...
    it('keyProvider return Promise', function () {
      var eos = Eos({ keyProvider: new _promise2.default(function (resolve) {
          resolve(wif);
        }) });
    ...
    it('create asset', async function () {
      ...
      var auth = { authorization: 'eosio.token' };
    ...
    it('multi-action transaction Promise.reject rollback', function () {
      var eos = Eos({ signProvider: signProvider });
      return eos.transaction(function (tr) {
        return _promise2.default.reject('rollback');
      }).then(function () {
        throw 'expecting rollback';
      }).catch(function (error) {
        assert(/rollback/.test(error), error);
